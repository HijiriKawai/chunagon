...
function selfAssignToUpdateAndInplace(node) {
  ...
  if (right.operator === '+') {
    ...
    if (another !== undefined) {
      ...
      if (
        another.type === 'UnaryExpression' &&
        another.operator === '-' &&
        another.argument.value === 1
      ) {
        return {
          type: 'UpdateExpression',
          start: 0,
    ...
    if (isSameValue(left, right.left)) {
      if (
        right.right.type === 'UnaryExpression' &&
        right.right.operator === '-' &&
        right.right.argument.value === 1
      ) {
        return {
          type: 'AssignmentExpression',
          start: 0,
...
function isSameValue(left, right) {
  ...
  if (left.type === 'MemberExpression')
    return isSameValue(left.object, right.object) && isSameValue(left.property, right.property);
}
...
function ConvertAllCode(code) {
  const newNode = convertAll(Parser.parse(code, { ranges: false }));
...
function RunAssertions(question, node_) {
  ...
  parsedAssertions.forEach((assertion) => {
    ...
    if (
      ...
      (!assertion.assertion.hasOwnProperty('none') && !assertion.assertion.matched)
    ) {
